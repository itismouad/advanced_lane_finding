...
The images are different which allow us to have a robust understanding of the various effects of the lense on our images. To effectively calibrate the camera, we use the `findChessboardCorners` function to detect corner coordinates (status is `1` if there is one else `0`).

Drawing them with `drawChessboardCorners` allows us to check we effectively found the corners on our calibration images :

![alt text][calib_draw]

All those corners points (for each image) are all the data we need to calibrate the camera. Indeed, the calibration step uses the `calibrateCamera` function and these object (actual flat chessboard) and image (distorted image) points to calculate a camera matrix, distortion matrix, etc. We can use then use these outputs in the `undistort` function to undistort different images (see below).
...
Using the color selection techniques such S-channel extraction in the HLS color space or white and yellow colors in the HLS and HSC color spaces, we are able to identify pretty well the lane lines.
...
Eventually, to determine the curvature of the lane lines, a polynomial was calculated on the meters space to be used here to calculate the curvature :
